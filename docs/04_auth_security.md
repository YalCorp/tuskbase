...
- `tuskbase bridge` lets local MCP clients use Tuskbase-managed credentials over stdio instead of requiring per-shell token environment variables.
- `/healthz` remains unauthenticated and reports the active auth policy and auth source.
- `tuskbase setup` generates and stores local keys; env vars such as `TUSKBASE_API_KEY` and `TUSKBASE_AGENT_KEYS` remain manual overrides.
...
- local API key is required and generated by `tuskbase setup`; the default MCP client path reads it through `tuskbase bridge`,
...
- stdio bridge for normal local MCP client setup,
